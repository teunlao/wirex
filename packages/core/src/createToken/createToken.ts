...
import type { TokenOptions, TokenType } from './createToken.h.js';
...
export function createToken<Type = any>(name?: string, options?: { scope?: ScopeVariants }): BaseTokenInterface<Type>;
...
export function optional<Token extends TokenInterface<any>>(token: Token): { token: Token; optional: true } {
  return { token, optional: true };
}

export type ExtractTokenType<Token extends TokenInterface<any>> = Token extends TokenInterface<infer Type>
  ? Type
  : unknown;
